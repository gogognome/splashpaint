...
static List<Integer>f(List<List<Integer>>s,int x,int y){try{return s.stream().filter(t->t.contains(c(x, y))).findFirst().get();}catch(Throwable e){return null;}}
static int c(int x,int y){return y*12+x;}
static List<List<Integer>> a(InputStream s)throws Throwable{List<List<Integer>>splashes=new ArrayList<>();for(int i=0;i<144;i++){int y=i/12;int x=i%12;int b=0;while(b<79){b=s.read();}if(b==88)splashes.add(new ArrayList<>(Arrays.asList(c(x, y))));}return splashes;}
static String d(List<Integer> s){return String.format("%d => %s",s.size(),s.stream().map(n->"("+n%12+", "+n/12+")").reduce((t,u)->t+", "+u).get());}}
