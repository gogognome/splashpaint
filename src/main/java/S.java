...
static List<Integer>f(List<List<Integer>>s,int x,int y){try{return s.stream().filter(t->t.contains(coordinatesToInt(x, y))).findFirst().get();}catch(Throwable e){return null;}}
