import java.io.*;
import java.util.*;
public class S{public static void main(String[]a)throws Exception{S s=new S();System.out.println(s.d(s.c(s.b(s.a(System.in)))));}
...
List<List<Long>>b(List<List<Long>>s){for(int i=0;i<144;i++){int y=i/12;int x=i%12;if(f(s,x,y)!=null){for(int dx=x==0?0:-1;dx<=(x==11?0:1);dx++)for(int dy=y==0?0:-1;dy<=(y==11?0:1);dy++)if((dx!=0||dy!=0)&&f(s,x+dx,y+dy)!=null)u(s,x,y,x+dx,y+dy);}}return s;}
